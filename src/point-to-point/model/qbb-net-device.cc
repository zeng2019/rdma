...
namespace ns3 {
		...
		QbbNetDevice::GetTypeId(void)
	{
		static TypeId tid = TypeId("ns3::QbbNetDevice")
			...
			.AddAttribute("L2AckInterval",
				...
				UintegerValue(0),
				MakeUintegerAccessor(&QbbNetDevice::m_ack_interval),   //每收到m_ack_interval个包，返回一个ACK
				MakeUintegerChecker<uint32_t>())
		...
		QbbNetDevice::DequeueAndTransmit(void)
	{
		...
		Ptr<Packet> p;
		if (m_node->GetNodeType() == 0 && m_qcnEnabled) //网卡 QCN enable NIC   QCN成立时才会根据QCN算法计算的时间来出队列
		{
			p = m_queue->DequeueQCN(m_paused, m_nextAvail, m_findex_qindex_map);    //m_queues[0]最高优先级，先判断m_queues[0]有没有要发送的，若没有则进行轮询
		}
		else if (m_node->GetNodeType() == 0 && m_timelyEnable) { //m_queues[0]最高优先级，先判断m_queues[0]有没有要发送的，若没有则round-robin
		...
		{
			p = m_queue->DequeueNIC(m_paused);   //轮询
		}
		else   //switch, doesn't care about qcn, just send
		{
			//p = m_queue->Dequeue(m_paused);		//this is strict priority 

			p = m_queue->DequeueRR(m_paused);		//7是最高优先级，先严格优先级，若没找到则round-robin 
		}
		...
		if (p != 0)
		{
			...
			uint16_t protocol = 0;
			ProcessHeader(packet, protocol);  //去掉PPP首部
			packet->RemoveHeader(h);
			FlowIdTag t;
			if (m_node->GetNodeType() == 0) //I am a NIC, do QCN
			{								//QCN并不真正改变网卡速率，而是通过改变队列下次发送的时间来近似控制网卡速率
				uint32_t fIndex = m_queue->GetLastQueue();   //取得刚刚出队列的index
				if (m_rate[fIndex] == 0)			//rate initialization	
				{
					if (m_timelyEnable)
						m_rate[fIndex] = initRate;	 //m_bps: 网卡默认的发送速率, 每条流的初始速率
					else
						m_rate[fIndex] = m_bps;
				...
				if (m_qcnEnabled) {   //qcn
					...
					{
						if (m_rpStage[fIndex][i] > 0) {	//1: fr; 2: ai; 3: hi   //初始化为0，只有当enableQCN且收到第一个cnp之后m_rpStage才赋值大于0
							m_txBytes[fIndex][i] -= p->GetSize();
						}
						else
							m_txBytes[fIndex][i] = m_bc;  //< Tx byte counter
						if (m_txBytes[fIndex][i] <= 0)     //发送完字节计数器指定大小的数据，进入相应的阶段更新发送速率
						{
					...
					if (segment_sent[fIndex] >= rtt_num) {
						segment_sent[fIndex] = 0;  //重新置0
						double creditsDue = std::max(0.0, m_bps / m_rate[fIndex] * (rtt_num * 1024 - m_credits[fIndex]));
						Time nextSend = m_tInterframeGap + Seconds(m_bps.CalculateTxTime(creditsDue));  //m_tInterframeGap默认=0
				...
				if (result) {
					sent[fIndex] += p->GetSize();   //更新流发送的数据量
				}
			}
			else //I am a switch, do ECN if this is not a pause
			{
				...
				else
				{
					//switch ECN
					p->PeekPacketTag(t);
					uint32_t inDev = t.GetFlowId();  //接收该数据包的交换机网卡号
													 //由于交换机只有8个队列，等级相同的入同一队列，故入端口和出端口的qIndex相同
		...
		QbbNetDevice::Retransmit(uint32_t findex)
	{
		...
		//将m_queue->m_queue[i]清空，然后将m_sendingBuffer的数据加入m_queue->m_queue[i]
		...
		QbbNetDevice::Receive(Ptr<Packet> packet)
	{
		...
		ProcessHeader(p, protocol);   //去掉PPP首部
		...
		{ //This is not QCN feedback, not NACK, not ACK or I am a switch so I don't care
			...
			{
				packet->AddPacketTag(FlowIdTag(m_ifIndex));   //m_ifIndex交换机的网卡号，即收到该数据包的网卡号码
				if (m_node->GetNodeType() == 0) //NIC
				{
					if (ipv4h.GetProtocol() == 17)	//look at udp only  网卡接收到一个数据包
					{
						//std::cout << "网卡收到数据包" << std::endl;
						...
						SeqTsHeader sth;
						p->PeekHeader(sth);
						uint64_t ts;   //收到的数据包带有timeStamp，将该timeStamp放在返回的ACK包中
						...
						for (i = 0; i < m_ecn_source->size(); ++i)
						{
							...
							{
								found = true;
								if (ecnbits != 0 && Simulator::Now().GetMicroSeconds() > m_qcn_np_sampling) //m_qcn_np_sampling默认=0
								{
						...
						if (!found)
						{
							...
							CheckandSendQCN(tmp.source, tmp.qIndex, tmp.port); //每个新到达的流隔cnp_interval调用该函数，用来判断是否发生cnp
																			   //将该优先级的流pause
						}
						...
						if (x == 2) //generate NACK
						{
							...
							SeqTsHeader sth;       //实现timely 
							if (m_qcnEnabled || m_timelyEnable)    //ack中SeqTsHeader的pg没什么用处，用到的是ack中qbbHeader的pg
								sth.SetPG(0);   //最高优先级
							else
								sth.SetPG(qCnt - 1);
							if (m_timelyEnable) {
						...
						else if (x == 1 || x == 3)   //产生ACK， x == 3 表示 Duplicate
						{
							if (x == 3)
							...
							qbb.SetPG(GetPriority(p->Copy()));  //返回的ACK的优先级，应该与收到数据包的优先级一致
			...
			else // If this is a Pause, stop the corresponding queue
			{
				...
				m_paused[qIndex] = true;  //暂停优先级为qIndex的流
				if (pauseh.GetTime() > 0)
				{
		...
		{
			qbbHeader qbbh;
			p->Copy()->RemoveHeader(qbbh);   //此时，p已经去掉ppp,ip首部
			...
			uint32_t key = i;
			SeqTsHeader sth;
			packet->PeekHeader(sth); //此处要调用PeekHeader或者Copy()函数，直接调用RemoveHeader()函数会报错！！！
		...
		/*	if (!m_timelyEnable) {
				...
				if (prev_rtt[key] == 0)
					...
					double new_rtt_diff = new_rtt - prev_rtt[key];  //此处不可用无符号整数new_rtt_diff
					prev_rtt[key] = new_rtt;
					rtt_diff[key] = (1 - weight_rtt_diff) * rtt_diff[key] + weight_rtt_diff * new_rtt_diff; //此处不可用无符号整数rtt_diff，可能有误差long long<-double
					double normalized_gradient = (double)rtt_diff[key] / minRTT[key];   //minRTT值得商榷
					if (new_rtt < rtt_low) {
						m_rate[key] = m_rate[key] + step;
						N = 0;   //考虑要不要去掉或者改成N++
								 //std::cout << "直接加速" << std::endl;
					}
...
/*
					...
					if (m_rate[key] > m_bps) {  //速率不能大于网卡速率
						m_rate[key] = m_bps;
					}
					else if (m_rate[key] < m_minRate) {
						m_rate[key] = m_minRate;   //100Mbps   最低速率
					}
				}
...
*/
		...
		else if (ipv4h.GetProtocol() == 0xFC)//ACK on NIC
		{
			...
			int64_t ts = sth.GetTs().GetTimeStep();
			uint32_t i;
			for (i = 1; i < m_queue->m_fcount; i++) {   //找到对应的流
				if (m_findex_qindex_map[i] == qIndex && m_findex_udpport_map[i] == port)
					break;
			}
			if (i >= m_queue->m_fcount) {
				std::cout << "ERROR in qbb-net-device.cc 'ACK on NIC': 找不到对应的流！" << std::endl;
			}
			uint32_t key = i;
			if (!m_timelyEnable) {
				new_rtt = Simulator::Now().GetTimeStep() - ts;
			...
			*/
				if (prev_rtt[key] == 0)
					...
					prev_rtt[key] = new_rtt;
					rtt_diff[key] = (1 - weight_rtt_diff) * rtt_diff[key] + weight_rtt_diff * new_rtt_diff; //此处不可用无符号整数rtt_diff，可能有误差long long<-double
					double normalized_gradient = (double)rtt_diff[key] / m_min_rtt;   //minRTT值得商榷
					if (new_rtt < rtt_low) {
						m_rate[key] = m_rate[key] + step;
						N = 0;   //考虑要不要去掉或者改成N++
								 //std::cout << "直接加速" << std::endl;
					//	std::cout << "小于rtt_low" << std::endl;
					}
					else if (new_rtt > rtt_hight) {
						double a = rtt_hight;
						double b = new_rtt;
						double tmp = 1 - weight_bida * (1 - a / b);
					//	std::cout << "大于rtt_high:" << tmp << std::endl;
						m_rate[key] = m_rate[key] * tmp;
						N = 0;
					//	std::cout << "大于rtt_high" << std::endl;
					}
					//Patched Timely
					/*	else {
					double weight;
					...
					//Original Timely
					else if (normalized_gradient <= 0) {   // 原来是 “<=”
				...
				rate = m_rate[key].GetBitRate();
				//	return; // ts !=0 时， ACK只是用来传达RTT
			}

			int seq = qbbh.GetSeq();
			uint32_t buffer_seq = GetSeq(m_sendingBuffer[key]->Peek()->Copy());  //复制m_sendingBuffer中的第一个packet

			if (m_ack_interval == 0)    //L2不支持不开启ACK功能
			{
			...
			else
			{
				if (!m_backto0)
				{
					while (seq > buffer_seq)       //若ACK确认的序列号大于m_sendingBuffer队首packet的序列号
					{
						m_sendingBuffer[key]->Dequeue();   //队首的packet收到ACK，可以删除
				...
				{
					uint32_t goback_seq = seq / m_chunk * m_chunk;   //中间计算过程可能产生浮点数
					while (goback_seq > buffer_seq)
					{
		...
		QbbNetDevice::Send(Ptr<Packet> packet, const Address &dest, uint16_t protocolNumber)
	{
		...
		if (IsLinkUp() == false) {        //判断链路是否处于up
			m_macTxDropTrace(packet);
			return false;
		}

		Ipv4Header h;
		packet->PeekHeader(h);   //复制packet的IP首部，并返回IP首部长度
		unsigned qIndex;
		...
		else    //udp-echo-client使用的协议为UDP（17）
		{
			Ptr<Packet> p = packet->Copy();
			p->RemoveHeader(h);

			if (h.GetProtocol() == 17)     //上层协议为UDP协议    
				qIndex = GetPriority(p);   // 此时p已经去掉了IP首部
			else
				qIndex = 1; //dctcp
		}

		Ptr<Packet> p = packet->Copy();
		AddHeader(packet, protocolNumber);  //增加PPP首部

		if (m_node->GetNodeType() == 0)     //网卡
		{
			if ((m_qcnEnabled || m_timelyEnable) && qIndex == qCnt - 1)
			{
				m_queue->Enqueue(packet, 0); //发送时，优先考虑队列0
			}
			else
			{
				Ipv4Header ipv4h;
				...
				if (m_sendingBuffer[i]->GetNPackets() == 8000)  //发送缓存过大时，丢掉队首的包
				{
					m_sendingBuffer[i]->Dequeue();
				}
				...
				{
					m_queue->Enqueue(packet, i);   //若支持QCN,TIMELY机制，则m_queue中队列数目最多为fcnt
				}
				else
				{
					m_queue->Enqueue(packet, qIndex);  //否则m_queue中队列数目为8   qIndex => i  aaa
				}
			}
			DequeueAndTransmit();
		}
		else //switch
		{
			...
			if (qIndex != qCnt - 1)			//not pause frame   不是QCN,NACK,ACK,PFC
			{
				FlowIdTag t;
				...
				{	//inDev接收网卡（端口）的id，m_ifIndex发送网卡（端口）的id
					//由于交换机只有8个队列，等级相同的入同一队列，故入端口和出端口的qIndex相同
				...
				{
					CheckQueueFull(inDev, qIndex); //check queue full  检测入端口，判断是否发送PFC
				}
			}
			else			//直接转发QCN,NACK,ACK,PFC
			{
	...
	{																	//只有交换机才会发送PFC
		...
		for (uint32_t j = 0; j < qCnt; j++)
		{
			if (!m_node->m_broadcom->m_pause_remote[inDev][qIndex])
				continue;
			if (m_node->m_broadcom->GetResumeClasses(inDev, qIndex))  //队列变小，通知上游解除PFC
			{
				Ptr<Packet> p = Create<Packet>(0);
		...
		QbbNetDevice::TransmitStart(Ptr<Packet> p)
	{
		...
		Time txTime = Seconds(m_bps.CalculateTxTime(p->GetSize()));   //计算网卡发送时间
		Time txCompleteTime = txTime + m_tInterframeGap;				//计算完成时间
		NS_LOG_LOGIC("Schedule TransmitCompleteEvent in " << txCompleteTime.GetSeconds() << "sec");
		Simulator::Schedule(txCompleteTime, &QbbNetDevice::TransmitComplete, this);   //经过txCompleteTime的时间后继续发送，此处模拟网卡的发送时延    
		...
		if (m_timelyEnable && m_node->GetNodeType() == 0) {
			...
			if (ipHeader.GetProtocol() == 17) {  //只在数据包中加入ts
				UdpHeader udpHeader;
		...
		if (result == false)
		{
			...
			m_sent += p->GetSize();  //已经发送的字节数
									 //	std::cout << "发送了: " << m_sent << std::endl;
		}
		return result;
	}
	...
	//接收方发送cnp，每m_qcn_interval微秒检测是否发送cnp
	void
		...
		QbbNetDevice::rpr_cnm_received(uint32_t findex, uint32_t hop, double fraction)
	{
		...
		{
			if (m_rpByteStage[findex][hop] != 0)  //字节计数器的周期
			{
		...
		m_rpByteStage[findex][hop] = 0;  //字节计数器的周期，收到QCN时置0
		m_rpTimeStage[findex][hop] = 0;  //计时器的周期
		...
		Simulator::Cancel(m_resumeAlpha[findex][hop]); //取消更新m_alpha的的事件
		m_resumeAlpha[findex][hop] = Simulator::Schedule(MicroSeconds(m_alpha_resume_interval), &QbbNetDevice::ResumeAlpha, this, findex, hop); //重新创建更新m_alpha的事件
		m_rpWhile[findex][hop] = m_rpgTimeReset;  //The rate increase timer at RP in microseconds
		Simulator::Cancel(m_rptimer[findex][hop]);
		m_rptimer[findex][hop] = Simulator::Schedule(MicroSeconds(m_rpWhile[findex][hop]), &QbbNetDevice::rpr_timer_wrapper, this, findex, hop);
		rpr_fast_recovery(findex, hop);  //RP重新回到快速恢复阶段，置m_rpStage[fIndex][hop] = 1;
	}
		...
		QbbNetDevice::ReceiverCheckSeq(uint32_t seq, uint32_t key)
	{
		...
		if (seq == expected)
		{
			...
			{
				m_milestone_rx[key] += m_ack_interval;   //加上返回ACK的间隔
