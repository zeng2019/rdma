	...
	*/
	...
	protected:
		...
		*/
		...
		uint64_t rtt_hight;
		uint64_t rtt_low;
		uint64_t step; //additive increase step
		uint32_t N; //used for HAI mode
		bool m_timelyEnable;*/


		//timely
		int64_t prev_rtt[fCnt] = { 0 };
		double rtt_diff[fCnt] = { 0 };
		int64_t rtt_hight;
		int64_t rtt_low;
		int64_t minRTT[pCnt] = { 0 };   //const???
		uint64_t m_min_rtt;
		uint64_t step;
		uint64_t rtt_ref;   //It is a constant value around which you want the RTT to be.
		int32_t N = 0; //used for HAI mode
		double weight_rtt_diff;
		double weight_bida;
		bool m_timelyEnable;
		uint64_t segment_sent[pCnt] = { 0 };
		uint64_t received[pCnt] = { 0 };
		uint64_t rtt_half[pCnt] = { 0 }; //数据包从发送端到接收端的时延
		uint16_t rtt_num = 16;  //timely：segment中数据包的个数
		uint64_t timely_segment;
		uint64_t rtt_noise;
		uint64_t rate_every_flow;   //每条流的初始速率
		TracedValue<int64_t> new_rtt = 0;
		TracedValue<int64_t> rate = 0;
		TracedValue<uint64_t> ack_cnt = 0;
		//pfc
	...
	public:
		uint64_t m_sent = 0;
		uint64_t sent[10] = { 0 };  //用来统计每条流发送的数据
		uint64_t m_rtt = 0;
		uint64_t rtt[10] = { 0 };  //保存每条流的rtt
