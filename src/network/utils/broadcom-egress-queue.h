...
namespace ns3 {
	...
	public:
		...
		Ptr<Packet> DequeueTimely(bool paused[], Time avail[], uint32_t m_findex_qindex_map[]);//Timely enable NIC
	...
	private:
		...
		Ptr<Packet> DoDequeueTimely(bool paused[], Time avail[], uint32_t m_findex_qindex_map[]);
		//for compatibility
		...
		uint32_t m_qlast;
		QueueMode m_mode;
		std::vector<Ptr<Queue> > m_queues; // uc queues   ∆‰ µ «DropTailQueue
		//For strict priority
