...
namespace ns3 {
	...
	Ptr<Packet>
		BEgressQueue::DoDequeueNIC(bool paused[])  //paused[] : whether a queue paused
													//轮询
	{
		NS_LOG_FUNCTION(this);
		if (m_bytesInQueueTotal == 0)
		...
		BEgressQueue::DoDequeueRR(bool paused[]) //this is for switch only
	{
		...
		{
			for (qIndex = qCnt - 2; qIndex < qCnt; qIndex--) //strict policy  严格优先级 6,5,4,3,2,1,0
			{
	...
	Ptr<Packet>
		BEgressQueue::DoDequeueTimely(bool paused[], Time avail[], uint32_t m_findex_qindex_map[])
	{
		NS_LOG_FUNCTION(this);
		if (m_bytesInQueueTotal == 0)
		{
		...
		bool found = false;
		uint32_t qIndex;
		if (m_queues[0]->GetNPackets() > 0)  //优先考虑队列0， in Timely, used for ACK
		{
			found = true;
			qIndex = 0;
		}
		...
		{
			for (qIndex = 1; qIndex <= m_fcount; qIndex++) //m_fcount 实际的流数目
			{
				if (!paused[m_findex_qindex_map[(qIndex + m_rrlast) % m_fcount]] && m_queues[(qIndex + m_rrlast) % m_fcount]->GetNPackets() > 0)
				{
					if (avail[(qIndex + m_rrlast) % m_fcount].GetTimeStep() > Simulator::Now().GetTimeStep()) //not available now
		...
		BEgressQueue::Enqueue(Ptr<Packet> p, uint32_t qIndex)
	{
		...
		if (retval)
		{
			...
			m_nBytes += size;   //继承自Queue的成员，此处为该BEgressQueues所有的DropTailQueue(Queue的子类)的m_nBytes之和
			m_nTotalReceivedBytes += size;

			m_nPackets++;	//继承自Queue的成员，此处为该BEgressQueues所有的DropTailQueue(Queue的子类)的m_nPackets之和
	...
	Ptr<Packet>
		BEgressQueue::DequeueTimely(bool paused[], Time avail[], uint32_t m_findex_qindex_map[])  //do all DequeueNIC does, plus Timely
	{
		NS_LOG_FUNCTION(this);
		Ptr<Packet> packet = DoDequeueTimely(paused, avail, m_findex_qindex_map);
		if (packet != 0)
		{
			NS_ASSERT(m_nBytes >= packet->GetSize());
			NS_ASSERT(m_nPackets > 0);
