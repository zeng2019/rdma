...
namespace ns3 {
	...
	BroadcomNode::BroadcomNode()
	{
		...
		m_dctcp_threshold = 40 * 1030;   //40    third.cc中没有调用函数设置该值
		m_dctcp_threshold_max = 400 * 1030; //400
	...
	bool
		BroadcomNode::ShouldSendCN(uint32_t indev, uint32_t ifindex, uint32_t qIndex) //进端口（网卡），出端口（网卡），出端口的队列号
	{
		if (qIndex == qCnt - 1)
			return false;
		if (qIndex == 1)	//dctcp
