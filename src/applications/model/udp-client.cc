...
UdpClient::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::UdpClient")
    ...
    .AddAttribute("CalRate",
				  "CalRate.",
				  UintegerValue(40),
				  MakeUintegerAccessor(&UdpClient::cal_rate),
				  MakeUintegerChecker<uint32_t>())
	/*.AddTraceSource("M_SENT",
		"m_sent",
		MakeTraceSourceAccessor(&UdpClient::m_sent))*/
  ;
  return tid;
}
...
UdpClient::Send (void)
{
  ...
  for (uint32_t i=0;i<dn;i++)
  {
	  ...
	  double tmp = (buffer*8.0-1500*8.0)/ cal_rate /1000000000*0.95; //0.95 is for conservative. assuming 40Gbps link.
  ...
  if (next_avail < 0.000005)
  {
	  ...
	  {
		  ++m_sent;
		//  std::cout << "m_send: " << m_sent << std::endl;
