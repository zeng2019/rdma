...
namespace ns3 {
		...
		UdpEchoClient::Send(void)
	{
		...
		if (next_avail < 0.000005)
		{
			...
			if (m_sentBytes < m_allowed && m_sentBytes / m_size < m_count)   //m_allowed在StartApplication中初设置为1个m_chunk，以后每接收到
			{																 //来自server的ACK便增加一个m_chunk（有点像Waiting ACK）
				if ((m_socket->Send(p)) >= 0)  //UdpSocket: socket::Send(packet) -> UdpSocketImpl::Send(packet, flag)
				{
					++m_sent;
